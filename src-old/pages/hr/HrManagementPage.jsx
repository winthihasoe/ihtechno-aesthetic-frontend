...
export default function HrManagementPage() {
  ...
  const usersEligibleForProfile = useMemo(
    () => users.filter((user) => !isCeoAccount(user)),
    [users],
  );
            ...
            <Card><CardContent>
              ...
              <Stack spacing={1}>
                <TextField select size="small" label="User" value={forms.staffUserId} onChange={(e) => setForms((p) => ({ ...p, staffUserId: e.target.value }))}>
                  {usersEligibleForProfile.map((user) => <MenuItem key={user.id} value={user.id}>{user.name}</MenuItem>)}
                </TextField>
