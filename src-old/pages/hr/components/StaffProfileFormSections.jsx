...
export default function StaffProfileFormSections({
  ...
  onUploadAvatar,
  onRemoveAvatar,
  /** Hide job position template and custom JD fields (details page sidebar card). */
  hideJobDescriptionFields = false,
  ...
  const probationEndPreview = showProbationFields
    ...
    : null;
  const selectedJobPosition = hideJobDescriptionFields
    ? null
    : jobPositions.find((item) => String(item.id) === String(form.jobPositionId));
    ...
    <Stack spacing={0}>
      ...
      <HrFormSection title="">
        ...
        {!hideJobDescriptionFields ? (
          <LabeledField id="staff-job-position" label="Job position template">
            <TextField
              ...
              helperText="Optional template used when no custom job description is set."
            >
        ...
        {!hideJobDescriptionFields ? (
          <LabeledField id="staff-jd-override" label="Custom job description">
            <RichTextEditor
              value={form.jobDescriptionOverride}
              onChange={(html) => patchForm({ jobDescriptionOverride: html })}
              helperText={
                selectedJobPosition
        ...
        {showAccountSection ? (
          <Stack direction={{ xs: "column", md: "row" }} spacing={1.25}>
            ...
            <LabeledField id="staff-deposit-amount" label="Deposit (MMK)">
              <TextField
                id="staff-deposit-amount"
                type="number"
                size="small"
                fullWidth
                ...
                helperText="Optional security deposit held by clinic"
              />
            </LabeledField>
          </Stack>
        ) : null}
        {showAccountSection && form.depositAmount !== "" && Number(form.depositAmount) > 0 ? (
          <LabeledField id="staff-deposit-release-date" label="Deposit release date">
            <TextField
              id="staff-deposit-release-date"
              type="date"
              size="small"
              fullWidth
              InputLabelProps={{ shrink: true }}
              value={form.depositScheduledReleaseDate ?? ""}
              onChange={(e) => patchForm({ depositScheduledReleaseDate: e.target.value })}
              helperText="Optional scheduled date to release deposit to staff"
            />
          </LabeledField>
        ) : null}
        <Stack direction={{ xs: "column", md: "row" }} spacing={1.25}>
          <LabeledField id="staff-annual-leave-days" label="Annual leave days">
            <TextField
              id="staff-annual-leave-days"
              type="number"
              size="small"
              fullWidth
              ...
              onChange={(e) =>
                patchForm({ annualLeaveDays: e.target.value.replace(/\D/g, "") })
              }
              helperText="Leave blank to use clinic default"
            />
          </LabeledField>
          <LabeledField id="staff-sick-leave-days" label="Sick leave days">
            <TextField
              id="staff-sick-leave-days"
              type="number"
              size="small"
              fullWidth
              placeholder="Clinic default"
              ...
              onChange={(e) =>
                patchForm({ sickLeaveDays: e.target.value.replace(/\D/g, "") })
              }
              helperText="Leave blank to use clinic default"
            />
          </LabeledField>
