...
import PasswordPolicyHint, {
  passwordMeetsPolicy,
} from "../components/auth/PasswordPolicyHint";
...
export default function UsersPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!editingUser && !passwordMeetsPolicy(form.password)) {
      pushToast({
    ...
    if (form.password && !passwordMeetsPolicy(form.password)) {
      pushToast({
        message: "Password does not meet the security requirements.",
        severity: "error",
      });
    ...
    try {
      if (editingUser) {
        ...
        if (passwordChanged && editingUser.id === authUser?.id) {
          pushToast({
  ...
  // CEO editing her own account: allow email/password changes but lock roles
  // so she cannot accidentally remove her own owner access.
  const editingSelfOwner =
    !!editingUser &&
    authUser?.id === editingUser.id &&
    hasRole(editingUser, "owner");
              ...
              filteredUsers.map((user) => (
                <TableRow key={user.id} hover>
                  ...
                  <TableCell align="right">
                    {hasRole(user, "owner") ? (
                      // CEO account: only the CEO herself may edit (email/password);
                      // it can never be deleted, and other staff see no actions.
                      authUser?.id === user.id ? (
                        <Tooltip title="Edit">
                          <span>
                            <IconButton
      ...
      >
        ...
        <DialogContent dividers>
          <Stack spacing={1.5} mt={0.5}>
            ...
            />
            <FormControl size="small" disabled={editingSelfOwner}>
              ...
              {editingSelfOwner && (
                <FormHelperText>
                  You cannot change your own CEO role.
                </FormHelperText>
              )}
            </FormControl>
            <TextField
