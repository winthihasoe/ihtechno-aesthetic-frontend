...
import ProductPickerMenuItem from "../../components/inventory/ProductPickerMenuItem";
    ...
    next.reception = {
      ...
      value:
        template.commission_value != null
          ? String(template.commission_value)
          : "",
    };
  }
  return next;
...
export default function TreatmentTemplateEditorPage() {
  ...
  const isManagementView = MANAGEMENT_ROLES.has(resolveUserPrimaryRole(user));
  ...
  const [perRoleCommission, setPerRoleCommission] = useState(
    emptyPerRoleCommissionState,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setTemplateUsage({
          packageItemsCount: Number(t.package_items_count || 0),
          treatmentsCount: Number(t.treatments_count || 0),
        });
        setName(t.name ?? "");
        ...
        const st =
          Array.isArray(t.steps) && t.steps.length ? t.steps : [emptyStep()];
        setSteps(
          st.map((s, i) => ({
            title: s.title ?? "",
        ...
        const tp =
          Array.isArray(t.template_products) && t.template_products.length
            ? t.template_products
            : [];
        setProductRows(
          tp.length
  ...
  const templateInUse =
    templateUsage.packageItemsCount > 0 || templateUsage.treatmentsCount > 0;
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      duration_minutes:
        durationMinutes === ""
          ? null
          : Math.max(1, parseInt(durationMinutes, 10) || 1),
  ...
  const handleDelete = async () => {
    if (!canManage || !isManagementView || isNew || templateInUse) return;
    if (!window.confirm("Delete this treatment template?")) return;
    setDeleting(true);
    try {
      await deleteTreatmentTemplate(id);
      pushToast({ message: "Template deleted.", severity: "success" });
      navigate("/inventory/treatment-templates");
    } catch (err) {
      pushToast({
  ...
  return (
    <Box>
      ...
      <Typography variant="h5" fontWeight={700} sx={{ mb: 2 }}>
        {isNew ? "New treatment template" : "Edit treatment"}
      </Typography>
      ...
      <Paper variant="outlined" sx={{ borderRadius: 2, p: 2.5 }}>
        <Stack spacing={2}>
          ...
          >
            Reception / sales is the same setting as the former single
            “commission” on this template (percent or fixed on the treatment
            line, then split between check-in and last follow-up). Older
            templates only using that field will show the value under Reception
            / sales; saving stores it in the new field and clears the duplicate
            legacy column.
          </Typography>
          ...
          <FormControlLabel
            ...
            label="Active (visible on Live Board)"
          />
          ...
          {!isNew && (
            <>
              ...
              <Typography
                ...
                sx={{ display: "block", mb: 1 }}
              >
                Linked forms must be satisfied in Treatment Preparation before
                this preset can move to the treatment room.
              </Typography>
                ...
                >
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      next[idx] = {
                        ...next[idx],
                        form_definition_id: e.target.value,
                      };
              ...
              >
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    next[idx] = {
                      ...next[idx],
                      default_quantity: e.target.value,
                    };
                    setProductRows(next);
                  }}
                  size="small"
                ...
                <IconButton
                  ...
                  onClick={() =>
                    setProductRows((r) => r.filter((_, i) => i !== idx))
                  }
          ...
          {canManage && (
            <Stack
              ...
              sx={{ pt: 2 }}
            >
              {!isNew && isManagementView ? (
                <Tooltip
                  title={
                    templateInUse
                      ? "In use by package/treatment. Deactivate instead."
                      : "Delete"
                  }
                >
                  <span>
                    <Button
                      ...
                      disabled={deleting || templateInUse}
                    >
              ...
              <Stack direction="row" spacing={1} justifyContent="flex-end">
                <Button
                  variant="outlined"
                  onClick={() => navigate("/inventory/treatment-templates")}
                >
                  Cancel
                </Button>
                <Button
