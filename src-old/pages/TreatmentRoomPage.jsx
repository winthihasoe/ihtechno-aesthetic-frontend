...
export default function TreatmentRoomPage() {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 2, pb: { xs: 4, sm: 3 } }}>
      <ConsultationRoomHeader
        title="Treatment Room"
        patient={patient ?? visit?.patient}
        visit={visit}
        photoCount={visit?.photos?.length ?? 0}
        plannedTreatmentCount={treatments.length}
        pairHeaderActionsWithCancel
        headerActions={
          canHandover ? (
            <Button
              variant="outlined"
        ...
        onPatientDetails={() => {
          const patientId = visit?.patient_id ?? visit?.patient?.id ?? patient?.id;
          if (!patientId) return;
      ...
      <Box sx={{ mb: 2 }}>
        <VitalSigns
          ...
          editable={canAccessTreatmentRoom(user, visit)}
          title="Vital Signs"
          subtitle="Review or update the latest measurements before treatment."
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, mb: 2 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
          Sessions planned in preparation appear here. Complete each session,
          then use <strong>Mark Done</strong> on the Live Board when all
