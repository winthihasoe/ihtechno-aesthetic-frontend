...
import VisitCancellationDetails from "./VisitCancellationDetails";
import { formatKyats } from "../../../utils/formatKyats";
import { parseVisitCancelReason } from "../../../utils/visitCancelReasonUtils";
  ...
  if (!sorted.length) {
    return (
      <PatientTabEmptyState
        ...
        description="Visit-level EMR details appear here after patient check-in and consultation activity."
        steps={[
          "Use Check-in from patient header.",
          "Complete consultation and treatment entries for the visit.",
          "Refresh this tab to review full visit details.",
        ]}
        previewFields={[
          "Visit status",
          "Consultation",
      ...
      >
        ...
        >
          {sorted.map((v) => {
            ...
            const cancelInfo =
              v.status === "cancelled" ? parseVisitCancelReason(v.cancel_reason) : null;
            return (
              <Card
                key={v.id}
                variant="outlined"
              ...
              >
                ...
                {cancelInfo ? (
                  <Typography
                    ...
                    noWrap
                  >
                    Cancelled: {cancelInfo.reason}
                    {cancelInfo.note ? ` — ${cancelInfo.note}` : ""}
                  </Typography>
                ) : null}
              </Card>
            );
        ...
        >
          {!selectedId && (
            <Typography variant="body2" color="text.secondary">
              Select a visit to view full EMR details.
            </Typography>
          )}
          {selectedId && loading && (
          ...
          {selectedId && !loading && detail && !error && (
            <Box>
              ...
              <VisitCancellationDetails visit={detail} sx={{ mb: 2 }} />
                ...
                {!detail.payment ? (
                  ...
                  >
                    ...
                    <Chip
                      size="small"
                      label={
                        detail.payment.status === "paid" ? "Paid" : "Unpaid"
                      }
                      color={
