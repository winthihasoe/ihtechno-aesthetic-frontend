...
export default function FormDetailsPage() {
  ...
  const handleToggleActive = async () => {
    ...
    try {
      ...
      setError(resolveApiError(err, "Failed to update form status."));
    } finally {
  ...
  const handleSelectVersion = async (versionId) => {
    ...
    try {
      ...
      setError(
        resolveApiError(err, "Failed to load selected version preview."),
      );
    } finally {
    ...
    <Box sx={{ maxWidth: 960, mx: "auto", pb: 2 }}>
      ...
      >
        ...
        <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
          ...
          />
          <Chip
            label={form?.published_version ? "Published" : "Draft only"}
            size="small"
            variant="outlined"
          />
          <Chip
          ...
          />
          {form?.is_usable ? (
            <Chip label="Usable in workflows" size="small" color="success" variant="outlined" />
          ) : null}
