...
import { canViewCarryoverStrip } from "../../utils/roleUtils";

const DRAWER_WIDTH = 220;

/**
 * @param {{ scrollContainerSx?: import("@mui/material").SxProps, showMobileSubNav?: boolean }} [props]
 */
export default function WorkspaceLayout({
  ...
  const { updateVisit, addVisit, fetchVisits, fetchCarryoverVisits, syncCarryoverVisit } =
    useLiveBoardStore();
  const { addNotification, hydrateNotifications, hydratedFromServer } =
  ...
  useEffect(() => {
    if (immersive) closeDrawer();
  }, [immersive, closeDrawer]);

  useEffect(() => {
    if (!token || hydratedFromServer) return;
    let cancelled = false;
    (async () => {
      try {
        const rows = await getNotifications({ unread_only: false, limit: 50 });
        if (!cancelled) hydrateNotifications(rows);
      } catch {
        // non-blocking
      }
    })();
  ...
  }, [token, hydratedFromServer, hydrateNotifications]);
  ...
  useEffect(() => {
    ...
    const handleStateChange = ({ current }) => {
      ...
      if (current === "connected") {
        ...
        if (canViewCarryoverStrip(u, settings)) {
          fetchCarryoverVisits({ background: true });
        }
      }
    };
  ...
  const showMobileBottomNav = sections.length > 1;
  ...
  return (
    ...
    >
      ...
      {showMobileSubNav ? <MobileSubNav items={mobileSubNavItems} /> : null}
