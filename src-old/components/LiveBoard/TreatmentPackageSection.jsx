  ...
  return (
    <Box sx={{ mb: 2 }}>
      ...
      <Typography
        ...
        sx={{ mb: 1 }}
      >
        Lines match this session&apos;s treatment preset when one is selected.
        Recording usage here deducts prepaid sessions; inventory still follows{" "}
        <strong>Mark session done</strong> (FIFO on products above).
      </Typography>
      {loading ? (
        <LoadingIndicator size={22} />
      ) : lines.length === 0 ? (
        <Alert severity="info" sx={{ fontSize: 13 }}>
          No active prepaid lines for this preset on this patient (or
          beneficiary) for this visit.
        </Alert>
      ) : (
        <Table size="small">
          <TableHead>
          ...
          <TableBody>
            {lines.map((row) => {
              const isPlanned = plannedPackageItemIds.includes(Number(row.id));
              return (
              <TableRow
                key={row.id}
                sx={isPlanned ? { bgcolor: "action.hover" } : undefined}
              >
                <TableCell sx={{ fontSize: 12 }}>
                  <Stack direction="row" spacing={0.75} alignItems="center">
                    <span>{row.patient_package?.package?.name ?? "—"}</span>
                    {isPlanned ? (
