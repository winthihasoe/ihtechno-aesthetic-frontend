...
import ProductPackSizeFields from "./ProductPackSizeFields";
...
function normalizeCreatePayload(form) {
  ...
  const sellingPrice = parseCommaAmount(form.selling_price);
  return {
    ...form,
    ...
    selling_price:
      form.selling_price === "" || !Number.isFinite(sellingPrice)
        ? null
        : sellingPrice,
    description: form.description.trim() || null,
    unit_id: unitId,
    stock_unit_id: sameUnit ? unitId : Number(form.stock_unit_id || unitId),
    base_unit_id: sameUnit ? unitId : Number(form.base_unit_id || unitId),
    base_per_stock_unit: sameUnit ? 1 : Number(form.base_per_stock_unit) || 1,
    track_open_units: Boolean(form.track_open_units),
    open_use_by_hours:
      form.track_open_units && form.open_use_by_hours
        ? Number(form.open_use_by_hours)
        : null,
  };
}
...
export default function NewProductDialog({
  ...
  useEffect(() => {
    ...
    const rawPrice = defaults.selling_price;
    const parsedPrice = parseCommaAmount(rawPrice);
    setForm({
      ...emptyNewProductForm(),
      ...defaults,
      selling_price:
        rawPrice === "" || rawPrice == null
          ...
          : Number.isFinite(parsedPrice)
            ? formatCommaAmountFromNumber(parsedPrice)
            : sanitizeCommaAmountInput(String(rawPrice)),
    });
    setNewUnitName("");
    setNewCategoryName("");
    setNewTypeName("");
          ...
          <Stack spacing={2} sx={{ pt: 1 }}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              <Box sx={{ flex: 1 }}>
                ...
                {types.find((t) => String(t.id) === form.product_type_id)?.code ===
                  "equipment" && (
                  <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: "block" }}>
                    For clinic-owned machines, use Finance → Fixed Assets. Equipment type is for consumable parts and shot-count items.
                  </Typography>
                )}
              </Box>
              <Box sx={{ flex: 1 }}>
