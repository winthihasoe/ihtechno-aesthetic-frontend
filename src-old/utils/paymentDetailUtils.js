...
export const normalizePaymentLines = (items) => {
  ...
  return items.lines.map((line) => {
    ...
    const unitPrice = hasPrice ? Number(rawPrice) : "";
    ...
    const base = {
      ...
      label: line?.label || "",
      qty,
      unit_price: Number.isFinite(unitPrice) ? unitPrice : hasPrice ? 0 : "",
...
/**
 * Mirrors backend FinancialService::initializeInvoiceTotals tax resolution:
 * explicit payment.tax_percentage wins; otherwise when settings.vat_enabled,
 * use settings.default_vat_percent (stored as vat_* in API; shown as Tax in UI).
 *
...
/**
 * @param {object} totalsAfterOrderDiscount - from applyOrderDiscountToTotals; `grand` is net before tax
...
/**
 * Preview remaining balance after the amount entered for this collection.
 * When no partial amount is entered, returns the full amount still due.
 */
export const previewRemainingAfterPartialPayment = ({
  payment,
  totalsGrand = 0,
  ...
  if (!Number.isFinite(paidThis) || paidThis <= 0) {
    return amountDue;
  }
  return Math.max(0, Number((amountDue - paidThis).toFixed(2)));
};
