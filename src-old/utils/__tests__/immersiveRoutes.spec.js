...
describe("isImmersiveWorkspacePath", () => {
  it("matches consultation, treatment, and preparation rooms", () => {
    expect(
    ...
    expect(
      isImmersiveWorkspacePath("/admin/visits/99/preparation-room"),
    ).toBe(true);
  });

  it("does not match standard workspace routes", () => {
    expect(isImmersiveWorkspacePath("/admin/live-board")).toBe(false);
    expect(isImmersiveWorkspacePath("/owner/reports/finance")).toBe(false);
  });
});
