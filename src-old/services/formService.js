...
/** Published, activated consent + questionnaire definitions for visit workflows. */
...
export const getFieldAudit = (formId, fieldId) =>
  apiClient
    .get(`/forms/${formId}/fields/${fieldId}/audit`)
    .then((r) => r.data);
...
export const getLatestVisitFormResponses = async (
  ...
  const scopedForms = (forms || []).filter(
    (f) =>
      (f.is_usable ?? (f.is_active && f.published_version)) &&
      formTypes.includes(f.form_type),
  );
