...
export const getCarryoverVisits = async () => {
  const { data } = await apiClient.get("/visits", {
    params: { carryover: true, today: false },
  });
  return data;
};

export const runLiveboardCloseDay = async (date) => {
  const { data } = await apiClient.post("/liveboard/close-day", date ? { date } : {});
  return data;
};
...
export const cancelCarryoverVisit = async (id, payload) => {
  const { data } = await apiClient.post(`/visits/${id}/cancel`, payload);
  return data;
};
