...
function doctorRoleLabel(role) {
  if (role === "dermatologist") return "Dermatologist";
  if (role === "medical_officer" || role === "doctor") return "Medical Officer";
  return role ?? "";
}

export default function WaitingPanel({ visit, user, onVisitUpdated }) {
  ...
  const loadStaff = useCallback(async () => {
    ...
    try {
      ...
      pushToast({
        message: resolveApiError(err, "Failed to load doctors."),
        severity: "error",
      });
      setDoctors([]);
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      pushToast({ message: "Doctor assigned.", severity: "success" });
    } catch (err) {
      pushToast({
        message: resolveApiError(err, "Failed to assign doctor."),
        severity: "error",
      });
    } finally {
      setSaving(false);
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
      <Stack spacing={1.25}>
        ...
        <Typography variant="body2">
          <strong>Primary concern:</strong> {visit?.notes ?? "—"}
        </Typography>
        <Typography variant="body2">
          <strong>Follow-up:</strong> {followUpLabel}
        </Typography>
        <Typography variant="body2">
          <strong>Waiting time:</strong> {waitingMinutes} min
        </Typography>
        ...
        {canAssign && (
          <Box sx={{ pt: 1 }}>
            ...
            >
              Only the assigned Medical Officer or Dermatologist can start consultation from the board.
            </Typography>
            {loadingStaff ? (
              <Box
              ...
              <Stack spacing={1.5}>
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    ...
                    label="Medical Officer / Dermatologist"
                    ...
                    renderValue={(val) => {
                      ...
                      const tag = doctorRoleLabel(d.role);
        ...
        {!canAssign && assignedName && (
          <Typography variant="body2" color="text.secondary" sx={{ pt: 0.5 }}>
            <strong>Assigned doctor:</strong> {assignedName}
          </Typography>
        )}
      </Stack>
    </Paper>
