...
export default function PatientCard({
  ...
  const pendingToId = visit.check_in_handover_to_id ?? visit.check_in_handover_to?.id;
  const pendingByName =
  ...
  const formatDurationWithUnit = (minutes) => {
  ...
  const durationLabel =
    visit.status === "waiting"
      ? `${formatDurationWithUnit(waitingMinutes)} waiting`
      : `${formatDurationWithUnit(stageMinutes)} in ${getVisitStatusConfig(visit.status).label}`;
  ...
  const canOpenByPanelRule = canUseLiveboardButton(user, visit, "open_panel", liveboardRules);
  const canOpen = canOpenByPanelRule;
