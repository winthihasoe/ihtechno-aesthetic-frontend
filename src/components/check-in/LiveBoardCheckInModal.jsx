...
export default function LiveBoardCheckInModal({
  ...
  useEffect(() => {
    if (!open || !bootstrapPatientId) return;
    let cancelled = false;
    (async () => {
      try {
        const p = await getPatient(bootstrapPatientId);
        if (cancelled) return;
        ...
        if (!cancelled) {
          pushToast({
            message: resolveApiError(
              err,
              "Could not load patient for check-in.",
            ),
            severity: "error",
          });
        }
  ...
  }, [open, bootstrapPatientId, onBootstrapConsumed, pushToast]);
  ...
  const handleCreatePatient = () => {
    onClose();
    navigate("/patients/new?returnTo=/live-board");
  };
          ...
          {!restrictPatientRegistration && customerMode === "new" && (
            <Box>
              <Typography
                ...
                sx={{ mb: 1.5 }}
              >
                Register a new patient with intake, then start check-in for this
                patient.
              </Typography>
