...
export default function PaymentDetailPage() {
  ...
  const invoiceCustomerLine = useMemo(() => {
    const name = resolvePaymentCustomerName(payment, customerName);
    return name ? `Customer name - ${name}` : "Customer name -";
  ...
  const load = async () => {
    ...
    try {
      ...
      const labTestsData = await getLabTests({ active_only: true }).catch(
        () => [],
      );
      if (isNewInvoiceMode) {
        setPayment(null);
      ...
      setLabTestOptions(Array.isArray(labTestsData) ? labTestsData : []);
  ...
  const handleMakePaymentAndPrint = async () => {
    ...
    try {
      ...
      const txResult = await addPaymentTransaction(wid, {
        amount: collection.amount,
        transaction_method_id: selectedTransactionMethodId,
      });
      let refreshed = await getPaymentById(wid).catch(() => txResult);
      // Ensure full settle is persisted as paid (detail + invoice list).
      if (
        paymentCollectionMode !== "partial" &&
        String(refreshed?.status || "").toLowerCase() !== "paid"
      ) {
        const total = Number(
        ...
        refreshed = await updatePayment(wid, {
          status: "paid",
          paid_amount: total,
          balance: 0,
  ...
  const activeTransactionMethods = useMemo(() => {
    const active = transactionMethods.filter(
      (m) =>
        m.status === "active" ||
        m.is_active === true ||
        String(m.status || "").toLowerCase() === "active",
  ...
  const receiptMeta = useMemo(() => {
    ...
    if (paymentCollectionMode === "partial") {
      const amt = Number(partialPaymentAmount || 0);
      const paidThis = Number.isFinite(amt) ? amt : 0;
      const amountDue = invoiceAmountDueBeforeNextPayment(pay, grand);
      const remaining =
        amountDue > 0
          ? Math.max(0, Number((amountDue - paidThis).toFixed(2)))
          : null;
      const due =
        String(partialDueDate || "").trim() !== ""
    ...
    if (st === "partial") {
      ...
      return {
        receiveViaLabel: label,
        paymentMode: "partial",
        partialAmount: paidSoFar > 0 ? paidSoFar : null,
        remainingAmount: Number.isFinite(balance) ? balance : null,
        dueDateLabel: due,
      };
    }
