...
export default function StaffProfileCreatePage() {
  ...
  useEffect(() => {
    Promise.all([getUsers(), getDepartments(), getStaffs()])
      .then(([usersPayload, departmentsPayload, staffsPayload]) => {
  ...
  const usersWithoutProfile = users.filter((user) => !user.staff_profile);
