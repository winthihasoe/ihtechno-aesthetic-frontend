...
export default function HrGrievancePage() {
  ...
  const [filters, setFilters] = useState({ status: "", recipient_type: "" });
  const [replyDrafts, setReplyDrafts] = useState({});
  const isOwner = hasRole(user, "owner");
  const canManageGrievances = isOwner || hasRole(user, "hr");
  const hasActiveFilters = Boolean(filters.status || filters.recipient_type);
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getStaffGrievances(filters);
      setRows(res.data || []);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <HrPageShell
      ...
      subtitle="Grievance"
      guide={[
        "Staff-raised concerns, including anonymous submissions.",
        "Reply and move the status (open → in review → resolved) to close the loop.",
        "Anonymous entries hide the staff name but still allow a follow-up thread.",
      ]}
    >
      {loading ? (
      ...
      {!loading && rows.length === 0 ? (
        <GrievanceInboxEmptyState
          ...
          isOwner={isOwner}
          onClearFilters={() => setFilters({ status: "", recipient_type: "" })}
        />
      ) : null}
