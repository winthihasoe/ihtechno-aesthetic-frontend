...
import { getEncounters } from "../../services/emrRegistryService";
...
export default function EncountersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(resolveApiError(err, "Could not load consultations."));
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      week: rows.filter((r) => dayjs(r.date).valueOf() >= startOfWeek.valueOf()).length,
      withRx: rows.filter((r) => r.has_prescription).length,
    };
  }, [rows]);
  ...
  return (
    <Box>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            Every consultation across the clinic — treatment concern, assessment
            and outcome. Open any record to review the full chart.
          </Typography>
        </Box>
      </Stack>
      ...
      <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2 }, mb: 2, border: `1px solid ${alpha(theme.palette.primary.main, 0.12)}` }}>
        <TextField
          size="small"
          placeholder="Search patient, assessment, concern, doctor…"
          value={search}
          ...
          <Table size="medium" stickyHeader>
            ...
            <TableBody>
              {loading ? (
                ...
                filtered.map((row) => {
                  const statusConfig = getVisitStatusConfig(row.status);
                  return (
                    <TableRow
                      key={row.id}
                      hover
                    ...
                    >
                      ...
                      <TableCell sx={{ maxWidth: 240 }}>
                        <Typography variant="body2" noWrap title={row.chief_complaint}>
                          {row.chief_complaint || "—"}
                        </Typography>
                      </TableCell>
                      <TableCell sx={{ maxWidth: 240 }}>
                        <Stack direction="row" spacing={0.75} alignItems="center">
                          <Typography variant="body2" sx={{ fontWeight: 600 }} noWrap title={row.diagnosis}>
                            {row.diagnosis || "—"}
                          </Typography>
                          {row.has_prescription && (
                            <Tooltip title="Prescription issued">
