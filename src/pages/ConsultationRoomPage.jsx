...
function LabeledTextField({ title, inputProps, ...props }) {
  return (
    <Box>
      <Typography
        variant="caption"
      ...
      <TextField
        {...props}
        inputProps={{ "aria-label": title, ...inputProps }}
      />
    </Box>
  );
}
...
export default function ConsultationRoomPage() {
  ...
  const buildPayload = (overrides = {}) => ({
    ...
    session_fee_enabled: Boolean(form.session_fee_enabled),
    session_fee_amount:
    ...
    session_fee_foc: Boolean(form.session_fee_foc),
    notes: form.notes,
    condition_snapshot: {
  ...
  const previousConsultations = useMemo(
    () =>
      priorVisits
        .map((v) => ({
          visit: v,
          consultation: v.consultations?.[0] ?? v.consultation ?? null,
        }))
        .filter((row) => row.consultation),
    [priorVisits],
  );
  ...
  const handleRemovePlanned = async (id) => {
    const okay = window.confirm("Delete this treatment preset from the plan?");
    if (!okay) return;
    setPlanBusy(true);
    setPlanError("");
    try {
        ...
        >
          <Paper variant="outlined" sx={sectionCardSx}>
            <SectionTitle
              ...
              subtitle="Primary concern, skin profile, vitals, examination and assessment."
            />
            <Stack spacing={1.5}>
              <LabeledTextField
              ...
              >
                <FormLabel>Fitzpatrick</FormLabel>
                <RadioGroup
                  row
                ...
                <Typography variant="caption" color="text.secondary">
                  1: very fair · 2: fair · 3: medium · 4: olive/brown · 5: dark
                  brown · 6: deepest tone
                </Typography>
              </FormControl>
              ...
              <Box sx={fieldGridSx}>
                <LabeledTextField
                  title="BP"
                  size="small"
                  value={form.vital_sign_bp}
                  onChange={onField("vital_sign_bp")}
                  fullWidth
                />
                <LabeledTextField
                  title="Pulse"
                  size="small"
                  value={form.vital_sign_pulse}
                  onChange={onField("vital_sign_pulse")}
                  fullWidth
                />
                <LabeledTextField
                  title="Temp"
                  size="small"
                  value={form.vital_sign_temp}
                  onChange={onField("vital_sign_temp")}
                  fullWidth
                />
                <LabeledTextField
                  title="SpO2"
                  size="small"
                  value={form.vital_sign_spo2}
                  onChange={onField("vital_sign_spo2")}
                  fullWidth
                />
              </Box>
              <LabeledTextField
              ...
              <Box sx={{ display: "flex", gap: 2, flexDirection: "column" }}>
                ...
                />
                <LabeledTextField
                  title="Assessment (secondary, comma-separated)"
                  size="small"
                ...
                {selectedQuestionnaireFormIds.length > 0 ? (
                  <Stack spacing={1}>
                    {selectedQuestionnaireFormIds.map((formId) => {
                      ...
                      const draft = questionnaireDraftsByFormId[String(formId)];
                      const isOpen =
                        String(expandedQuestionnaireFormId) === String(formId);
                      const isLoaded =
              ...
              >
                ...
                >
                  {thumbnails.length ? (
                    ...
                    <Typography variant="caption" color="text.secondary">
                      No {label.toLowerCase()} thumbnails yet.
                    </Typography>
                  )}
                </Stack>
              </Paper>
          ...
          <Stack spacing={1.5}>
            <Box sx={fieldGridSx}>
              <LabeledTextField
                ...
                size="small"
                value={form.session_fee_amount}
                onChange={onField("session_fee_amount")}
                fullWidth
              />
              <LabeledTextField
                title="Discount %"
            ...
            <FormGroup row sx={{ gap: 1 }}>
              <FormControlLabel
                control={
                  <Switch
                    checked={Boolean(form.session_fee_enabled)}
              ...
              <FormControlLabel
                control={
                  <Switch
                    checked={Boolean(form.session_fee_foc)}
                    onChange={onField("session_fee_foc")}
                  />
                }
                label="FOC"
              />
