...
const sectionLabel = (value) =>
  FIELD_SECTION_OPTIONS.find((o) => o.value === value)?.label ?? value ?? "other";
...
const blankField = () => ({
  ...
  optionsText: "", // raw textarea value for editing
  required: false,
  section: "other",
});
    ...
    const load = async () => {
      ...
      try {
        ...
        const versionPayload = draftVersionId
          ...
          : await getForm(id);
        const version = versionPayload?.version ?? draftPayload?.version ?? null;
        const ff = versionPayload?.fields ?? draftPayload?.version?.fields ?? [];
